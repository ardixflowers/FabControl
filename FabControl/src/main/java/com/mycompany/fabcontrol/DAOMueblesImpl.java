...
public class DAOMueblesImpl extends Conexion implements DAOMuebles {
    ...
    public void registrar(Muebles muebles) throws Exception {
        try {
            this.Conectar();
            PreparedStatement st = this.conexion.prepareStatement("INSERT INTO muebles(nombre, tipo, material, color, dimensiones, peso, precio, stock, disponibles) VALUES(?,?,?,?,?,?,?,?,?);");
    ...
    public void modificar(Muebles muebles) throws Exception {
        try {
            this.Conectar();
            PreparedStatement st = this.conexion.prepareStatement("UPDATE muebles SET nombre = ?, tipo = ?, material = ?, color = ?, dimensiones = ?, peso = ?, precio = ?, stock = ?, disponibles = ? WHERE id = ?");
            st.setString(1, muebles.getNombre());
            st.setString(2, muebles.getTipo());
            st.setString(3, muebles.getMaterial());
            st.setString(4, muebles.getColor());
